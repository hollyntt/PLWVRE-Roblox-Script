...
local function PLVSMVWVRE_Menu()
    ...
    --// Settings (Defined outside, named 'ESP')
    ESP = {
    ...
    --// Logic (Runs automatically via IIFE)
    (function()
        --// Variables
        local Players = game:GetService("Players")
        ...
        local function createEsp(player)
            local esp = {
                boxOutline = create("Square", { Color = ESP.BoxOutlineColor, Thickness = 3, Filled = false }),
                box = create("Square", { Color = ESP.BoxColor, Thickness = 1, Filled = false }),
                name = create("Text", { Color = ESP.NameColor, Outline = true, Center = true, Size = 13 }),
                healthOutline = create("Line", { Thickness = 3, Color = ESP.HealthOutlineColor }),
                health = create("Line", { Thickness = 1 }),
                distance = create("Text", { Color = Color3.new(1, 1, 1), Size = 12, Outline = true, Center = true }),
                tracer = create("Line", { Thickness = ESP.TracerThickness, Color = ESP.TracerColor, Transparency = 1 }),
                boxLines = {},
            }
        ...
        local function removeEsp(player)
            ...
            -- Remove all main drawing objects
            local drawingKeys = {"tracer", "boxOutline", "box", "name", "healthOutline", "health", "distance"}
            ...
            if esp.boxLines then
                for _, line in ipairs(esp.boxLines) do
                    if line and typeof(line) == "userdata" and line.Remove then
            ...
            -- Remove skeletonlines
            if esp.skeletonlines then
                for _, lineData in ipairs(esp.skeletonlines) do
                    if lineData and lineData[1] and typeof(lineData[1]) == "userdata" and lineData[1].Remove then
        ...
        local function updateEsp()
            for player, esp in pairs(cache) do
                ...
                if character and (not ESP.Teamcheck or (team and team ~= localPlayer.Team)) then
                    local rootPart = character:FindFirstChild("HumanoidRootPart")
                    local head = character:FindFirstChild("Head")
                    local humanoid = character:FindFirstChild("Humanoid")
                    local isBehindWall = ESP.WallCheck and isPlayerBehindWall(player)
                    local shouldShow = not isBehindWall and ESP.Enabled
                    ...
                    if rootPart and head and humanoid and shouldShow then
                        ...
                        if onScreen then
                            ...
                            if ESP.ShowName and ESP.Enabled then
                            ...
                            if ESP.ShowBox and ESP.Enabled then
                                ...
                                elseif ESP.BoxType == "Corner Box Esp" then
                                    ...
                                    if #esp.boxLines == 0 then
                                        for i = 1, 16 do
                                            local boxLine = create("Line", { Thickness = 1, Color = ESP.BoxColor, Transparency = 1 })
                                    ...
                                    for i = 9, 16 do
                                        ...
                                        boxLines[i].Color = ESP.BoxOutlineColor
                            ...
                            if ESP.ShowHealth and ESP.Enabled then
                                ...
                                esp.health.Color = ESP.HealthLowColor:Lerp(ESP.HealthHighColor, healthPercentage)
                            else
                                esp.healthOutline.Visible = false
                                esp.health.Visible = false
                            end

                            if ESP.ShowDistance and ESP.Enabled then
                            ...
                            if ESP.ShowSkeletons and ESP.Enabled then
                                if #esp["skeletonlines"] == 0 then
                                    for _, bonePair in ipairs(bones) do
                                        local parentBone, childBone = bonePair[1], bonePair[2]
                                        local skeletonLine = create("Line", { Thickness = 1, Color = ESP.SkeletonsColor, Transparency = 1, Visible = false })
                                ...
                                for _, lineData in ipairs(esp["skeletonlines"]) do
                                    ...
                                    if character and character:FindFirstChild(parentBone) and character:FindFirstChild(childBone) then
                                        ...
                                        skeletonLine.Color = ESP.SkeletonsColor
                            ...
                            if ESP.ShowTracer and ESP.Enabled then
                                local tracerY
        ...
        -- Expose the update function to the ESP table so your main loop can call it
